...
# as lambdas + o secret compartilhado. Nenhuma das duas tem API Gateway
...
if [[ "$DESTROY" == "false" && "$SKIP_BUILD" == "false" ]]; then
  echo "==> Instalando dependências, testando e buildando a lambda..."
...
if $DESTROY; then
  echo ""
  echo "==> Destruindo as lambdas (auth-cpf + jwt-authorizer) + secret..."
  terraform -chdir="$DEPLOY_DIR" destroy $AUTO
  exit 0
fi

echo ""
echo "==> Provisionando as lambdas (auth-cpf + jwt-authorizer) + secret..."
terraform -chdir="$DEPLOY_DIR" apply $AUTO

echo ""
echo "==> Concluído. Próximo passo: infra addons vai ler estes ARNs e anexar"
echo "    as duas lambdas ao único API Gateway (rota /auth/cpf + authorizer)."
